...
router.get('/', async (req, res) => {
  ...
  try {
    console.log('Attempting to fetch cities from database...');
    // Get cities for dropdown
    const citiesResult = await pool.query('SELECT * FROM cities ORDER BY name');
    cities = citiesResult.rows;
    console.log('✅ Cities fetched from database:', cities.length);

    // Get real popular routes with actual data
...
// Booking page - redirect to seat selection
...
router.post('/booking', async (req, res) => {
  try {
    ...
    // Get schedule details for price and date validation
    ...
    // Insert booking with pending approval for non-cash payments
    // All bookings now require partner admin approval initially
    ...
    const smsMessage = `Your T-BUS booking ${bookingRef} is ${bookingStatus}. Seat: ${seat_number}. ${payment_method === 'cash' ? 'Pay at terminal.' : 'Awaiting payment approval.'}`;
...
async function sendNotification(phone, message, type, bookingRef) {
  try {
    ...
    console.log(`${type.toUpperCase()} to ${phone}: ${message}`);
  } catch (error) {
